 ...
 */
@WebServlet(name = "CadastrarTrabalho", urlPatterns = {"/CadastrarTrabalho"})
@MultipartConfig
public class CadastrarTrabalho extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
              if (request.getPart("imagem") != null) {
                try {
                    ...
                    if (oTrabalho.getImagem()== null) {
                        oTrabalho.setImagem(request.getParameter("imagemProduto64"));
                    }
                } catch (IOException e) {
            ...
            if (dao.cadastrar(oTrabalho)) {
                mensagem = "Trabalho cadastrado com sucesso!";
            } else {
                mensagem = "Problemas ao cadastrar Trabalho. Verifique os dados informados e tente"
                        + " novamente! ";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("ListarTrabalho").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Problemas no Servlet ao cadastrar Trabalho! Erro: " + ex.getMessage());
        }
    }

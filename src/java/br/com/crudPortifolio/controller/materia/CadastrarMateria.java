 ...
 */
@WebServlet(name = "CadastrarMateria", urlPatterns = {"/CadastrarMateria"})
public class CadastrarMateria extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (dao.cadastrar(oMateria)) {
                mensagem = "Matéria cadastrado com sucesso!";
            } else {
                mensagem = "Problemas ao cadastrar Matéria. Verifique os dados informados e tente"
                        + " novamente! ";
            }
            request.setAttribute("mensagem", mensagem);
            request.getRequestDispatcher("ListarMateria").forward(request, response);
        } catch (Exception ex) {
            System.out.println("Problemas no Servlet ao cadastrar Matéria! Erro: " + ex.getMessage());
        }
    }

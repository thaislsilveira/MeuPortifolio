 ...
 */
@WebServlet(name = "ExcluirMateria", urlPatterns = {"/ExcluirMateria"})
public class ExcluirMateria extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            try{
                ...
                if(dao.excluir(idMateria)){
                    mensagem = "Matéria excluída com sucesso!!";
                }else{
                    mensagem = "Problemas ao excluir Matéria!!";
                }     
                request.setAttribute("mensagem", mensagem);
                request.getRequestDispatcher("ListarMateria").forward(request, response);
            }catch(Exception ex){
                System.out.println("Problemas no Servlet ao excluir Matéria!! Erro : " + ex.getMessage());
            }
        }
    }
